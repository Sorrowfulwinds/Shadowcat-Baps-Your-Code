/*
/datum/controller/subsystem/job/proc/ProcessRoundstartPlayer(mob/M, datum/prototype/role/legacy_job/J, loadout = TRUE, client/C)
	// autodetect
	if(!C)
		C = M.client
	if(!J && M.mind)
...
/datum/controller/subsystem/job/proc/ProcessLatejoinPlayer(mob/M, datum/prototype/role/legacy_job/J, loadout = TRUE, client/C)
	// autodetect
	if(!C)
		C = M.client
	if(!J && M.mind)
...
/datum/controller/subsystem/job/proc/GreetPlayer(mob/M, datum/prototype/role/legacy_job/J, latejoin, client/C)
...
/datum/controller/subsystem/job/proc/EquipPlayer(mob/M, datum/prototype/role/legacy_job/J, loadout = TRUE, datum/preferences/prefs, announce, latejoin, client/C)
	if(!istype(J))
		J = GetJobAuto(J)
	ASSERT(istype(J))
...
/datum/controller/subsystem/job/proc/PostJoin(mob/M, datum/prototype/role/legacy_job/J, client/C, latejoin)
	// job handling
	if(J)
...
/datum/controller/subsystem/job/proc/AnnounceJoin(mob/M, datum/prototype/role/legacy_job/J, client/C)
 ...
 * - force - bypass checks
 */
/datum/controller/subsystem/job/proc/Assign(datum/mind/M, datum/prototype/role/legacy_job/J, latejoin = FALSE, force = FALSE)
	if(ismob(M))
		var/mob/_M = M
		M = _M.mind
	if(!istype(M))
...
/datum/controller/subsystem/job/proc/CanAssign(M, datum/prototype/role/legacy_job/J, latejoin)
...
/datum/controller/subsystem/job/proc/SendToLatejoin(mob/M, client/C = M.client, faction, job, method, override)
	if(!override && M.mind?.assigned_role)
		var/datum/prototype/role/legacy_job/J = SSjob.GetJobName(M.mind.assigned_role)
		if(J)
			faction = J.faction
			job = J.GetID()
...
/datum/controller/subsystem/job/proc/SendToRoundstart(mob/M, client/C, datum/prototype/role/legacy_job/J)
...
/datum/controller/subsystem/job/proc/handle_auto_deadmin_roles(client/C, rank)
	if(!C?.holder)
		return TRUE
	var/datum/prototype/role/legacy_job/job = GetJobAuto(rank)
	if(!job)
		return
