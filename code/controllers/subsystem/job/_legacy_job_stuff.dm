...
/datum/controller/subsystem/job/proc/FindOccupationCandidates(datum/prototype/role/legacy_job/job, level)
...
/datum/controller/subsystem/job/proc/FillHeadPosition()
	for(var/level in JOB_PRIORITY_HIGH to JOB_PRIORITY_LOW step -1)
		for(var/command_position in SSjob.get_job_titles_in_department(DEPARTMENT_COMMAND))
			var/datum/prototype/role/legacy_job/job = RSroles.legacy_job_by_title(command_position)
			if(!job)
				continue
...
/datum/controller/subsystem/job/proc/DivideOccupations()
	...
	if(SSticker && SSticker.triai)
		for(var/datum/prototype/role/legacy_job/A in RSroles.legacy_all_job_datums())
			if(A.title == "AI")
				A.spawn_positions = 3
				break
	...
	var/datum/prototype/role/legacy_job/assist = new DEFAULT_JOB_TYPE ()
...
/datum/controller/subsystem/job/proc/LoadJobs(jobsfile) //ran during round setup, reads info from jobs.txt -- Urist
	...
	for(var/job in jobEntries)
		...
		if(name && value)
			var/datum/prototype/role/legacy_job/J = RSroles.legacy_job_by_title(name)
			if(!J)	continue
