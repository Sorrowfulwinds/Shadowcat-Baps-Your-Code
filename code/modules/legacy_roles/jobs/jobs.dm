...
/proc/get_job_datums()
	...
	var/list/all_jobs = typesof(/datum/prototype/role/legacy_job)

	for(var/A in all_jobs)
		var/datum/prototype/role/legacy_job/job = new A()
		if(!job)	continue
		occupations += job
...
/proc/get_alternate_titles(var/job)
	...
	for(var/datum/prototype/role/legacy_job/J in jobs)
		if(J.title == job)
			titles = J.alt_titles
