...
//Compliance Collar
/obj/item/clothing/accessory/collar/bomb
	name = "Bomb collar"
	desc = "A collar used to enforce compliance."
	icon_state = "collar_shk0"
	item_state = "collar_shk_overlay"
	overlay_state = "collar_shk_overlay"
...
/obj/item/clothing/accessory/collar/bomb/Initialize(mapload)
	. = ..()
	radio_connection = radio_controller.add_object(src, frequency, RADIO_CHAT) // Makes it so you don't need to change the frequency off of default for it to work.

/obj/item/clothing/accessory/collar/bomb/Destroy() //Clean up your toys when you're done.
	radio_controller.remove_object(src, frequency)
	radio_connection = null //Don't delete this, this is a shared object.
	return ..()

/obj/item/clothing/accessory/collar/bomb/proc/set_frequency(new_frequency)
	radio_controller.remove_object(src, frequency)
	frequency = new_frequency
	radio_connection = radio_controller.add_object(src, frequency, RADIO_CHAT)

/obj/item/clothing/accessory/collar/bomb/Topic(href, href_list)
	if(usr.stat || usr.restrained())
		return
	if(((istype(usr, /mob/living/carbon/human) && ((!( SSticker ) || (SSticker && SSticker.mode != "monkey")) && usr.contents.Find(src))) || (usr.contents.Find(master) || (in_range(src, usr) && istype(loc, /turf)))))
		usr.set_machine(src)
...
//BOOM
/obj/item/clothing/accessory/collar/bomb/receive_signal(datum/signal/signal)
	if(!signal || signal.encryption != code)
		return
	...
	if(on)
		...
		if(ismob(loc.loc))
			M = loc.loc // This is about as terse as I can make my solution to the whole 'collar won't work when attached as accessory' thing.
		to_chat(M,"<span class='danger'>You feel lightheaded</span>")
		var/datum/effect_system/spark_spread/s = new /datum/effect_system/spark_spread
		s.set_up(3, 1, M)
		s.start()
		explosion(M, 1, 1, 0, 1)
...
/obj/item/clothing/accessory/collar/bomb/attack_self(mob/user as mob, flag1)
	if(!istype(user, /mob/living/carbon/human))
		return
	user.set_machine(src)
	var/dat = {"<TT>
