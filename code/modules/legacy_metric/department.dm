...
/datum/legacy_metric/proc/guess_job(mob/M)
	...
	if(R) // They got a record, now find the job datum.
		var/datum/prototype/role/legacy_job/J = RSroles.legacy_job_by_title(R.fields["real_rank"])
		if(istype(J))
			return J

	// Try the mind.
	if(M.mind)
		var/datum/prototype/role/legacy_job/J = RSroles.legacy_job_by_title(M.mind.assigned_role)
		if(istype(J))
			return J

	// Last ditch effort, check for job assigned to the mob itself.
	var/datum/prototype/role/legacy_job/J = RSroles.legacy_job_by_title(M.job)
	if(istype(J))
		return J
...
/datum/legacy_metric/proc/role_name_to_department(var/role_name)
	var/datum/prototype/role/legacy_job/J = RSroles.legacy_job_by_title(role_name)
	if(istype(J))
...
/datum/legacy_metric/proc/get_people_with_job(job_type, cutoff = 75)
	...
	for(var/M in GLOB.player_list)
		var/datum/prototype/role/legacy_job/their_job = guess_job(M)
