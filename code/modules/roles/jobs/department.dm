...
// todo: refactor
/datum/department
	var/const/name = "NOPE"		// Name used in UIs, and the index for the department assoc list in SSjob.
	var/const/short_name = "NO"	// Shorter name, used for things like external Topic() responses.
	var/const/color = "#000000"	// Color to use in UIs to represent this department.
	...
	var/const/sorting_order = 0	// Used to sort departments, e.g. Command always being on top.
	var/const/visible = TRUE		// If false, it should not show up on things like the manifest or ID computer.
	var/const/assignable = TRUE	// Similar for above, but only for ID computers and such. Used for silicon department.
	var/const/centcom_only = FALSE
