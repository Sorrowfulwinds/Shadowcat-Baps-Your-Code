...
/datum/datacore/proc/get_manifest(monochrome, OOC)
	...
	if(OOC)
		for(var/datum/data/record/t in hidden_general)
			...
			var/datum/prototype/role/legacy_job/J = RSroles.legacy_job_by_title(real_rank)
			if(J?.offmap_spawn)
				off[name] = rank
...
/datum/datacore/proc/manifest_modify(var/name, var/assignment, var/rank)
	...
	for(var/datum/prototype/role/legacy_job/J in all_jobs)
...
/datum/datacore/proc/manifest_inject(var/mob/living/carbon/human/H)
	if(H.mind && !player_is_antag(H.mind, only_offstation_roles = 1))
		...
		var/hidden
		var/datum/prototype/role/legacy_job/J = RSroles.legacy_job_by_title(H.mind.assigned_role)
